...
func CloneVolumeByPipe(SourcePool libvirt.StoragePool, SourceVolName string,
	...
	// 获取源卷信息
	_, rCapacity, _, err := libvirtd.Connection.StorageVolGetInfo(sourceVol)
	if err != nil {
	...
	if err != nil {
		// 如果复制失败，尝试删除目标卷
		_ = libvirtd.Connection.StorageVolDelete(newVol, 0)
...
func copyVolData(sourceVol, destVol libvirt.StorageVol) error {
	...
	go func() {
		...
		err := libvirtd.Connection.StorageVolDownload(sourceVol, pipeWriter, 0, capacity, 0)
		errChan <- err
	}()
	...
	go func() {
		...
		err := libvirtd.Connection.StorageVolUpload(destVol, pipeReader, 0, capacity, 0)
		errChan <- err
	}()
