...
func GetDomainXMLDesc(domain libvirt.Domain) (string, error) {
	xmlDesc, err := GetConnection().DomainGetXMLDesc(domain, 0)
	if err != nil {
...
func DefineDomain(xmlDesc string) (libvirt.Domain, error) {
	domain, err := GetConnection().DomainDefineXML(xmlDesc)
	if err != nil {
...
func UpdateDomain(domain libvirt.Domain, xmlDesc string) (libvirt.Domain, error) {
	...
	newDomain, err := GetConnection().DomainDefineXMLFlags(xmlDesc, 0)
	if err != nil {
